...
from utils.utils import dict2str, init_seed, get_model
...
def objective_function(config_dict=None, config_file_list=None, saved=True):
    ...
    # model evaluation
    test_result_u, test_result_i = trainer.evaluate(test_data, load_best_model=saved)
    test_result = OrderedDict()
    test_result['For user'] = test_result_u
    test_result['For item'] = test_result_i
        ...
        with open(output_file, "w") as fp:
            for params in self.params2result:
                ...
                valid_result_str = "\n".join([str(direct) + " : " + dict2str(value)
                                              for direct, value in
                                              self.params2result[params]["best_valid_result"].items()])
                fp.write(
                    "Valid result:\n"
                    + valid_result_str
                    + "\n"
                ...
                test_result_str = "\n".join([str(direct) + " : " + dict2str(value)
                                              for direct, value in
                                              self.params2result[params]["test_result"].items()])
                fp.write(
                    "Test result:\n"
                    + test_result_str
                    + "\n\n"
